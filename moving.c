...
/*   Updated: 2024/10/16 17:54:17 by aal-hawa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_move_player(int y, int x, t_info *info)
{
	...
	if (info->map[y][x] == 'C')
	{
		info->cnt_collc--;
		info->map[y][x] = '0';
		mlx_put_image_to_window(info->mlx, info->win, info->img_wlk, x * info->sz, y * info->sz);
	}
	mlx_put_image_to_window(info->mlx, info->win, info->img_plr, x * info->sz, y * info->sz);
	info->y_plr = y;
	info->x_plr = x;
	info->steps++;
	print_steps(info);
...
void	move_up(t_info *info)
{
	...
	if (is_moved == 1)
		mlx_put_image_to_window(info->mlx, info->win, info->img_wlk,  info->x_plr * info->sz, (info->y_plr + 1) * info->sz);
}
...
void	move_right(t_info *info)
{
	...
	if (is_moved == 1)
		mlx_put_image_to_window(info->mlx, info->win, info->img_wlk, ( info->x_plr - 1) * info->sz, info->y_plr  * info->sz);
}
